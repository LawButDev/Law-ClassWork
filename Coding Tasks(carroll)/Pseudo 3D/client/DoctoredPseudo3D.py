...
class enemy(rotent):
    def __init__(self,x_ref,y_ref,rot,enid):
        super().__init__(x_ref,y_ref,unit//2,unit//2,(playersp),rot)
        self.id = enid
    def update(self):
        super().update()    
    ...
    def update(self):
        ...
        if self.shooting:
            ...
            if self.lastshot <= tick:
                ...
                #todo - rewrite shooting logic to raycasting so i can actually damage players -- all done
...
## -- player character and enemy classes - npcs and the like
        ...
        for f in range(planewidth):
            ...
            for col in depog_mask:
                if col.type == "sprite":
                    ...
                    #  from there you can find the pixel that point covers and render the sprite
            ...
            # - sorts the sprite list so that it is from back to front cos otherwise it breaks lol
            j = 0
            sortedsprite = []
