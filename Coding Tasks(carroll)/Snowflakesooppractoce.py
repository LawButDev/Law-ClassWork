...
class snowblocker():
    ...
    def collision(self,snowdrop):
        if self.mask.overlap_area(snowdrop.mask,(snowdrop.x,snowdrop.y)) >= 20 and snowdrop.size > 10:
            snowdrop.regen()
...
class fence(snowblocker):
    def __init__(self,y,xstart,xend):
        super().__init__(xstart,y)
        self.xend = xend
        self.postheight = 15
    def update(self):
        placeholder = "filler"
    def draw(self):
        self.fencecalc()
        pygame.draw.polygon(self.surface,(220,220,220),[(self.originx,self.y-self.postheight//2),(self.originx + self.count*10,self.y-self.postheight//2),(self.originx+self.count*10,self.y-self.postheight//2-(self.postheight//8)),(self.originx,self.y-self.postheight//2-(self.postheight//8))])
        for i in range(self.count+1):
            self.drawpost(self.originx + i*10)
        super().draw()
    def fencecalc(self):
        self.count = abs(self.xend - self.x)//10
        if self.x < self.xend:
            self.originx = self.x
        else:
            self.originx = self.xend
    def drawpost(self,postx):
        pygame.draw.polygon(self.surface,(250,250,250),[(postx-3,self.y),(postx-3,self.y-self.postheight),(postx,self.y-self.postheight-5),(postx+3,self.y-self.postheight),(postx+3,self.y)])
...
fencepost = fence(size[1],11*size[0]//20 - 5,12*size[0]//20)
furnishing.append(fencepost)

fencepost = fence(size[1],9*size[0]//20,8*size[0]//20)
furnishing.append(fencepost)

home = House(size[0]//2,size[1])
furnishing.append(home)

post = lamppost(16*size[0]//20,size[1],0)
furnishing.append(post)


tree = Tree(1*size[0]//20,size[1],2)
furnishing.append(tree)
tree = Tree(3*size[0]//20,size[1],3)
furnishing.append(tree)
